...
run_path=$PWD'/trial/ldc_exp/'
valid_folder='nist03/nist03.'

# for l in en zh; do for f in $data_path/*.$l; do if [[ "$f" != *"test"* ]]; then sed -i "$ d" $f; fi;  done; done
# for l in en zh; do for f in $data_path/*.$l; do perl tools/tokenizer.perl -a -no-escape -l $l -q  < $f > $f.atok; done; done
